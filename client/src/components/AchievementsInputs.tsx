  ...
  useEffect(() => {
    if (
      (collectData.achievements.defaultAchieve &&
        Object.keys(mainAchievement).length == 0) ||
      collectData.achievements.dynamicAchieve ||
      Object.keys(achievement).length == 0
    ) {
      setMainAchievement(
        collectData.achievements.defaultAchieve as { [key: string]: string }
      );
  ...
  return (
    <div className=" min-h-[70vh]">
      ...
      {achievement.map((desc, index) => {
        return (
          <div key={index} className="flex flex-wrap gap-3 mt-2 pl-4">
            <RealInput
              type="text"
